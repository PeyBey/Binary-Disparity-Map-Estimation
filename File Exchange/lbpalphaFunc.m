function [dispMap1,gr,E]=lbpalphaFunc(l,r,ws,disparity,filt,filtWindow,nbit,GR,scale)
[e1,e2]=size(l);
...
alpha=45;
selector1=randi(nbit,2*ws);
selector2=randi(nbit,floor(sqrt(2*(ws/2)^2)));
selector3=randi(nbit,floor(sqrt(2*(ws/2)^2)));
